...
const exportPdfFunc = `
  const exportToPDF = async () => {
    ...
    try {
      ...
      // VIP Border - Outer border VERY BOLD
      ...
      doc.setFont("times", "bold");
      // Align exact center
      doc.text(\`ACADEMIC TIMETABLE - \${className.toUpperCase()}\`, pageWidth / 2, 41, { align: 'center', charSpace: 1.5 });
      ...
      const tableData = timesArray.map((time, rowIndex) => {
        const rowData: any[] = [{ content: formatTimeAmPm(time), styles: { cellWidth: 28 } }];
        daysOfWeek.forEach(day => {
