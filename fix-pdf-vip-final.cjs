...
const exportPdfFunc = `
  const exportToPDF = async () => {
    ...
    try {
      ...
      // VIP Border - Double border for premium look
      ...
      if(logoLoaded) {
         ...
         doc.setFont("times", "bold");
         doc.text(\`ACADEMIC TIMETABLE - \${className.toUpperCase()}\`, pageWidth / 2, 38, { align: 'center', charSpace: 1.5 });
      } else {
         doc.setFontSize(30);
         ...
         doc.setTextColor(80, 80, 80);
         doc.setFont("times", "bold");
         doc.text(\`ACADEMIC TIMETABLE - \${className.toUpperCase()}\`, pageWidth / 2, 40, { align: 'center', charSpace: 1.5 });
      }
