...
# Replace types
content = content.replace('TeacherRequest[]', 'ChatMessage[]')
content = content.replace('setTeacherRequests', 'setMessages')
content = content.replace('teacherRequests', 'messages')

# Replace states
content = re.sub(
    r'const \[isComposingRequest, setIsComposingRequest\] = useState\(false\);\n  const \[requestFormData, setRequestFormData\] = useState\(\{ type: \'Exchange Class\', message: \'\', email: \'\', name: \'\' \}\);\n  const \[requestReplyData, setRequestReplyData\] = useState<Record<string, string>>\(\{\}\);\n  const \[isSubmittingRequest, setIsSubmittingRequest\] = useState\(false\);',
...
content = content.replace('fetchTeacherRequests', 'fetchMessages')
content = content.replace('const fetchMessages = async () => {', 'const fetchMessages = async () => {')
content = content.replace('customFetch("/api/requests");', 'customFetch("/api/messages");')
content = content.replace('setMessages(data);', 'setMessages(data);')

# Replace request handlers
content = re.sub(
    r'const handleRequestSubmit = async \(e: React\.FormEvent\) => \{[\s\S]*?const handleDeleteRequest = async \(id: string\) => \{[\s\S]*?  \};',
...
chat_modal_regex = r'\{isRequestsModalOpen && \([\s\S]*?\{isLogsOpen && \('

new_chat_modal = '''{isRequestsModalOpen && (
            ...
            >
              <div className="flex justify-between items-center p-6 border-b border-black/5 dark:border-white/5 shrink-0 bg-white/50 dark:bg-black/50 backdrop-blur-md">
                <h2 className="text-xl font-bold tracking-tight flex items-center gap-2">
                  ...
                  {isAdmin ? (selectedChatUser ? \`Chat with \${messages.find(m => m.senderEmail === selectedChatUser)?.senderName || selectedChatUser}\` : "User Chats") : "Chat with Admin"}
                </h2>
              ...
              <div className="flex-1 overflow-hidden flex flex-col bg-white/30 dark:bg-black/30">
                {isAdmin && !selectedChatUser ? (
                  <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {(() => {
                      const userEmails = Array.from(new Set(messages.map(m => m.senderRole === 'user' ? m.senderEmail : m.receiverEmail).filter(e => e !== 'admin')));
                      ...
                      return userEmails.map(email => {
                        const userMsgs = messages.filter(m => m.senderEmail === email || m.receiverEmail === email);
                        const latestMsg = userMsgs[userMsgs.length - 1];
                        const unreadCount = userMsgs.filter(m => m.senderEmail === email && !m.readByAdmin).length;
                        return (
                          <div 
                            key={email} 
                            onClick={() => {
                          ...
                          >
                            <div>
                              <div className="font-bold text-lg">{latestMsg.senderRole === 'user' ? latestMsg.senderName : (messages.find(m => m.senderEmail === email)?.senderName || email)}</div>
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 flex flex-col">
                      {messages.filter(m => isAdmin ? (m.senderEmail === selectedChatUser || m.receiverEmail === selectedChatUser) : true).length === 0 ? (
                        ...
                        messages.filter(m => isAdmin ? (m.senderEmail === selectedChatUser || m.receiverEmail === selectedChatUser) : true).map((msg) => {
...
content = re.sub(chat_modal_regex, new_chat_modal, content)
